...
function createView (existingViewId, id, webPreferencesString, boundsString, events) {
  ...
  function handleExternalProtocol (e, url, isInPlace, isMainFrame, frameProcessId, frameRoutingId) {
    var knownProtocols = ['http', 'https', 'file', 'darkfang', 'about', 'data', 'javascript', 'chrome'] // TODO anything else?
...
if (settings.get('customUserAgent')) {
  ...
  newUserAgent = defaultUserAgent.replace(/Darkfang\/\S+\s/, '').replace(/Electron\/\S+\s/, '')
}
...
setTimeout(function () {
  if (!settings.get('v1_23_keychainMigrationComplete')) {
    keytar.findCredentials('DarkFang saved password').then(function (results) {
