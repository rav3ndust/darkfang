...
require('./createPackage.js')('mac', { arch: toArch(platform) }).then(function (packagePath) {
  if (platform === 'arm64') {
    execSync('codesign -s - -a arm64 -f --deep ' + packagePath + '/Darkfang.app')
  }
  ...
  var output = fs.createWriteStream('dist/app/darkfang-v' + version + '-mac-' + platform + '.zip')
  var archive = archiver('zip', {
    zlib: { level: 9 }
  })

  archive.directory(path.resolve(packagePath, 'Darkfang.app'), 'Darkfang.app')
